...
#[get("/move_turns_at_freq?<hz>&<forward>&<turns>")]
fn move_turns_at_freq(pi_state: State<SharedPi>, hz: f64, forward: bool, turns :u64) -> String {
    let pi_mutex = &mut pi_state.inner();
    let pi = &mut *pi_mutex.lock().unwrap();
    crate::motion::move_turns_at_freq(pi, AxisDirection::X, forward, hz, turns);
    "move_turns_at_freq complete".to_string()
}

#[get("/run_motor_test?<forward>&<acceleration_constant>&<number_of_turns>")]
fn run_motor_test(pi_state: State<SharedPi>, forward: bool, acceleration_constant : f64, number_of_turns: u64) -> String {
    let pi_mutex = &mut pi_state.inner();
    let pi = &mut *pi_mutex.lock().unwrap();
